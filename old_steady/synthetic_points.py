# Synthetic data test
# Successively adding more collocation points to the data and observe
...
save_path = "paper/synthetic/points/"
...
alpha = 0 # Regularization parameter for data points (set to zero) 
alpha_colloc = 1 * h_max_train**2 # Regularization parameter for added collocation points

# Define collocation points in x for each q value from the training data
q_colloc = q_list

# Train model using increasing numbers of collocation points
# No PDE residual evaluation for data points 
n_collocs = [0, 5, 10, 20, 40, 80, 160]
out_file.create_dataset('n_runs', data=len(n_collocs))

for ic, n_colloc in enumerate(n_collocs):
    ...
    groupname = "n_colloc_%d" %(ic)
    grp = out_file.create_group(groupname)
    grp.create_dataset('alpha_colloc', data=alpha_colloc)
    grp.create_dataset('n_colloc', data=n_colloc)
    if X_colloc is not None:
        grp.create_dataset('X_colloc', data=X_colloc)
